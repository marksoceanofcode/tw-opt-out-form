...
const OptOutForm = ({
  ...
  const onOptOutSubmit = async (data: any) => {
    setDisableFormButton(true) //Disable the form button right away

    //Replace with an actual API request for production
    const response = {
      message: "success",
      status: 200,
    }

    //Add any specific logic you would like here
    setResponseCode(response.status)

    if (response.status === 200) {
      setResponseMessage("You have been unsubscribed.")
    } else {
      setResponseMessage(
        "An error occurred processing your request. Please send an email to privacy@oursite.com"
      )
    }
  ...
  return (
    <div className="flex flex-col mx-4 rounded-lg shadow-lg shadow-slate-500/40 md:flex-row md:mx-6">
      <div className="bg-slate-100 border border-solid border-slate-200 flex flex-col justify-between p-16 rounded-tl-lg rounded-tr-lg w-full md:rounded-bl-lg md:rounded-tr-none md:w-2/5 md:min-h-[284px]">
      ...
      <div className="bg-white border border-solid border-slate-100 flex-auto p-16 rounded-br-lg rounded-bl-lg w-full md:rounded-bl-none md:rounded-tr-lg md:w-3/5">
        {showResponseMessage ? (
          <h3 className="text-center font-semibold mb-4 text-dark-gray md:text-left">
            {responseMessage}
          </h3>
        ) : (
          <>
                  ...
                  <Controller
                    ...
                    rules={{
                      ...
                      validate: (value: string) => {
                        ...
                        return (
                          cleaned.length === 10 ||
                          formErrorMessages.phone.invalid
                        )
                      },
                    }}
                    render={({ field: { onChange, onBlur, value, ref } }) => (
